...
| `mobile` | 是 | 充值手机号 |
| `faceValue` | 是 | 充值面额 |
| `product_type` | 否 | 开放接口枚举值为 `FAST` / `MIXED`，默认 `MIXED` |
...
| `province` | 自动识别出的省份 |
| `ispName` | 自动识别出的运营商 |
| `requestedProductType` | 请求的充值类型，默认 `MIXED` |
...
- `FAIL` 为必须保留的业务状态。
- 当前预存余额同步退款链路下，供应商失败或超时允许直接进入 `REFUNDING`。
- 当未来接入在线支付或异步退款时，标准主链路保留为 `FAIL -> REFUNDING -> REFUNDED`。
