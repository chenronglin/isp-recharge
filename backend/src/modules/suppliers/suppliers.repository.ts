...
export class SuppliersRepository {
  ...
  async upsertRechargeProduct(item: SupplierCatalogItem): Promise<ProductRecord> {
    const existingProduct = await this.findRechargeProductByBusinessKey(item);
    ...
    if (existingProduct) {
      const rows = await db<ProductRecord[]>`
        ...
        SET
          ...
          recharge_mode = ${item.rechargeMode},
          sales_unit = ${item.salesUnit ?? 'CNY'},
      ...
      if (!product) {
        throw new Error(`复用平台商品失败: ${existingProduct.id}`);
      }
